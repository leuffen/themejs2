# ThemeJS Sync Root Dir

## Zweck

Dieser Skill synchronisiert das aktuelle Projekt `themejs2`: die aktive Entwicklungsumgebung unter `docs/` mit dem wiederverwendbaren Projekt-Stub unter `_root/`. `docs/` ist die Quelle für aktuelle Entwicklungsänderungen; die bereits etablierte Struktur unter `_root/` ist die verbindliche Zielstruktur und darf beim Synchronisieren nicht wieder auf die historische `docs/`-Struktur zurückgebaut werden.
...
9. Änderungen an den für den Stub benötigten Root-Builddateien prüfen. `vite.config.ts` werden bei relevanten Änderungen aus dem Repository-Root nach `_root/` übernommen.
...
- Die `package.json` importiert jeweils nur die aktuelle version von `@leuffen/themejs2`. 

...
Jeder neue oder aktualisierte interne Pull Request löst `.github/workflows/deploy-pages-on-pull-request.yml` aus. Bei jedem Push auf den PR-Branch führt dieser Workflow automatisch `npm update` und `npm run build:site` (Vite-Build), anschließend den Jekyll-Build und das Pages-Deployment unter `https://tjs.gh.leuffen-demo.de/` aus. Bearbeite Theme-Änderungen in den SCSS-Quellen, niemals manuell in gebauten CSS-Dateien wie `docs/assets/dist/style.css`. Benötigte Build-Dateien werden ausschließlich durch den Build erzeugt; für die PR-Vorschau baut der Workflow sie selbst neu. Warte bei der Theme-Entwicklung nach jedem Push ausdrücklich auf den erfolgreichen Abschluss des zugehörigen Build- und Deploy-Laufs, bevor du das Ergebnis visuell bewertest; rechne mit circa 30 Sekunden als Richtwert und prüfe den tatsächlichen Actions-Status, statt nach einer festen Wartezeit Erfolg anzunehmen. Lade anschließend die betroffenen Seiten direkt über diese URL und vergleiche den gerenderten Stand in Desktop- und Mobilansichten visuell mit der Zielvorlage und den für den Auftrag relevanten Vergleichsständen. Bessere erkennbare Abweichungen innerhalb des Auftrags nach, pushe die Korrekturen in denselben Pull Request und wiederhole Warten, Laden und visuellen Abgleich, bis das Qualitätsziel erreicht ist oder ein konkreter Blocker beziehungsweise eine notwendige Nutzerentscheidung verbleibt. Die feste Pages-URL zeigt immer den zuletzt erfolgreich deployten Pull Request; verwende sie deshalb nur, wenn der zugrunde liegende Build-Lauf nachweislich zum aktuellen Head-Commit des bearbeiteten Pull Requests gehört und genau dessen Artifact erfolgreich deployt wurde. Bei fehlgeschlagenem oder noch laufendem Deploy darf ein älterer Preview-Stand nicht als aktuelles Ergebnis gewertet werden; dokumentiere im Abschluss den geprüften Commit, die aufgerufenen Seiten und verbleibende visuelle Abweichungen oder Blocker.
